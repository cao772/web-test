...
export function planAgentTools(rawCommand) {
  ...
  if (command.includes('最热') || command.includes('最高温')) {
    calls.push({ name: 'focus_hottest', arguments: {} })
    calls.push({ name: 'set_heatmap', arguments: { enabled: true } })
    return calls
  }

  if (command.includes('故障链') || command.includes('冷却异常')) {
    calls.push({ name: 'set_incident', arguments: { enabled: true } })
    calls.push({ name: 'set_network', arguments: { enabled: true } })
    calls.push({ name: 'set_airflow', arguments: { enabled: true } })
    calls.push({ name: 'focus_rack', arguments: { rack_id: 'RACK-07' } })
    return calls
  }

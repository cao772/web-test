...
import { buildDirectorSummary, DIRECTOR_INTRO, getDirectorMeta } from './directorRuntime'
...
function ToolCallLog({ items }) {
  return (
    <div className="tool-call-log">
      ...
      {items.length === 0 ? (
        <p className="tool-empty">执行指令或自动演示后，这里显示实际调用工具。</p>
      ) : items.slice(0, 4).map((item, index) => (
        <div className="tool-log-row" key={`${item.at}-${index}`}>
          <span className={`tool-state ${item.ok ? 'ok' : 'bad'}`} /><code>{item.name}</code><p>{item.summary}</p>
        </div>
      ))}
    </div>
  )
    ...
    <div className="demo-timeline">
      {AUTO_DEMO_STEPS.map((step, index) => (
        <div key={step.id} className={`demo-step ${index < stepIndex ? 'done' : ''} ${index === stepIndex && running ? 'active' : ''}`}>
          <span>{index + 1}</span>
          <div><strong>{step.label}</strong><small>{index === stepIndex && running ? '执行中' : index < stepIndex ? '已完成' : '等待'}</small></div>
        </div>
      ))}
    </div>
  )
...
function SnapshotCompare({ before, after }) {
  ...
  const rows = [
    ['入口温度', before?.temp, after?.temp, '°C'],
    ['GPU 负载', before?.gpu, after?.gpu, '%'],
    ['机柜功率', before?.power, after?.power, ' kW'],
    ['风扇转速', before?.fan, after?.fan, '%'],
  ]
  return (
    <div className="snapshot-compare">
      ...
      {rows.map(([label, a, b, unit]) => (
        <div className="compare-row" key={label}><span>{label}</span><b>{a ?? '--'}{a != null ? unit : ''}</b><i>→</i><strong>{b ?? '--'}{b != null ? unit : ''}</strong></div>
      ))}
...
function DirectorIntro({ visible }) {
  if (!visible) return null
  return (
    <div className="director-intro">
      <div className="director-intro-mark"><span /><i /></div>
      <p>{DIRECTOR_INTRO.eyebrow}</p>
      <h2>{DIRECTOR_INTRO.title}</h2>
      <div className="director-intro-line" />
      <span>{DIRECTOR_INTRO.subtitle}</span>
    </div>
  )
}

function DirectorCaption({ visible, stepIndex }) {
  if (!visible) return null
  const meta = getDirectorMeta(stepIndex)
  return (
    <div className="director-caption" key={meta.id}>
      <div className="director-caption-index">{String(stepIndex + 1).padStart(2, '0')}</div>
      <div>
        <p>{meta.eyebrow}</p>
        <h2>{meta.title}</h2>
        <span>{meta.subtitle}</span>
      </div>
      <small>{meta.shot}</small>
    </div>
  )
}

function FaultHud({ visible, rack, incident, mitigating, stepIndex }) {
  if (!visible || !rack) return null
  const severity = rack.status === 'critical' ? '严重' : rack.status === 'warning' ? '预警' : '恢复'
  return (
    <div className={`fault-hud ${rack.status === 'critical' ? 'critical' : ''}`}>
      <div className="fault-hud-head">
        <div><span className="fault-pulse" /><strong>故障定位 HUD</strong></div>
        <b>{severity}</b>
      </div>
      <div className="fault-path-row">
        <div><small>异常源</small><strong>CRAC-02</strong></div><i>→</i><div><small>影响设备</small><strong>RACK-07</strong></div>
      </div>
      <div className="fault-hud-grid">
        <div><span>入口温度</span><strong>{rack.temp.toFixed(1)}°C</strong></div>
        <div><span>GPU 负载</span><strong>{Math.round(rack.gpu)}%</strong></div>
        <div><span>机柜功率</span><strong>{rack.power.toFixed(1)} kW</strong></div>
        <div><span>处置状态</span><strong>{mitigating ? '降载中' : incident ? '传播中' : '恢复中'}</strong></div>
      </div>
      <div className="fault-hud-foot">STEP {String(stepIndex + 1).padStart(2, '0')} / {String(AUTO_DEMO_STEPS.length).padStart(2, '0')}</div>
    </div>
  )
}

function DirectorProgress({ visible, stepIndex }) {
  if (!visible) return null
  const percent = Math.min(100, ((stepIndex + 1) / AUTO_DEMO_STEPS.length) * 100)
  return (
...
function DirectorSummary({ visible, before, after, onReplay, onExit }) {
  if (!visible) return null
  const summary = buildDirectorSummary(before, after)
  if (!summary) return null
  return (
    <div className="director-summary-backdrop">
      <section className="director-summary-card">
        <div className="summary-check">✓</div>
        <p>MISSION COMPLETE · AUTOMATED RESPONSE</p>
        <h2>智能故障处置演示完成</h2>
        <span className="summary-conclusion">{summary.conclusion}</span>
        <div className="summary-metrics">
          <div><small>温度下降</small><strong>{summary.tempDrop > 0 ? '-' : '+'}{Math.abs(summary.tempDrop).toFixed(1)}°C</strong></div>
          <div><small>GPU 负载变化</small><strong>{summary.gpuDrop > 0 ? '-' : '+'}{Math.abs(summary.gpuDrop)}%</strong></div>
          <div><small>功率变化</small><strong>{summary.powerDrop > 0 ? '-' : '+'}{Math.abs(summary.powerDrop).toFixed(1)} kW</strong></div>
          <div><small>运行状态</small><strong>{STATUS[summary.beforeStatus].label} → {STATUS[summary.afterStatus].label}</strong></div>
        </div>
        <div className="summary-route">
          <span>异常发现</span><i>→</i><span>根因定位</span><i>→</i><span>节点检查</span><i>→</i><span>自动处置</span><i>→</i><span>恢复验证</span>
        </div>
        <div className="summary-actions">
          <button onClick={onReplay}>重新演示</button>
          <button className="secondary" onClick={onExit}>退出导演模式</button>
        </div>
      </section>
    </div>
  )
...
export default function V6App() {
  ...
  const [agentMessage, setAgentMessage] = useState('v0.6 导演模式就绪：支持自动字幕、故障 HUD、镜头编排和最终总结。')
  const [plannerSource, setPlannerSource] = useState('LOCAL')
  const [toolCalls, setToolCalls] = useState([])
  const [events, setEvents] = useState([
    { time: '16:20:12', level: 'info', text: '数字孪生 v0.6 导演模式就绪' },
    { time: '16:19:48', level: 'warning', text: 'RACK-07 温度处于预警区间' },
  ])
  const [demoRunning, setDemoRunning] = useState(false)
  const [demoStep, setDemoStep] = useState(0)
  const [demoMessage, setDemoMessage] = useState('点击“导演模式演示”进入可录屏的完整故障处置链。')
  const [beforeSnapshot, setBeforeSnapshot] = useState(null)
  const [afterSnapshot, setAfterSnapshot] = useState(null)
  const [directorMode, setDirectorMode] = useState(false)
  const [directorIntro, setDirectorIntro] = useState(false)
    ...
    const timer = window.setInterval(() => {
      ...
      setRacks((current) => current.map((rack) => {
        if (rack.id !== 'RACK-07') {
          return {
            ...rack,
            temp: Math.max(21.3, rack.temp + (Math.random() - 0.5) * 0.24),
            gpu: Math.min(97, Math.max(40, rack.gpu + (Math.random() - 0.5) * 1.6)),
          }
        }
        if (incident && !mitigating) {
          ...
          return {
            ...rack,
            temp,
            gpu: Math.min(99, rack.gpu + 0.5),
            fan: Math.min(100, rack.fan + 0.65),
        ...
        if (mitigating) {
          ...
          return {
            ...
            gpu,
            temp,
            fan: Math.min(96, rack.fan + 1.3),
            power: Math.max(9.4, rack.power - 0.16),
            status: temp > 35 ? 'critical' : temp > 30 ? 'warning' : 'healthy',
          }
        }
        if (!incident && rack.temp > 29.5) {
          ...
          return {
            ...rack,
            temp,
            gpu: Math.max(70, rack.gpu - 0.1),
            power: Math.max(9.8, rack.power - 0.08),
            status: temp > 30 ? 'warning' : 'healthy',
          }
        }
  ...
  const rack07 = racks.find((rack) => rack.id === 'RACK-07')
  ...
  const setIncidentMode = (enabled, source = 'manual') => {
    ...
    pushEvent(
      ...
      enabled
        ? `${source === 'demo' ? '自动演示' : 'Agent'} 激活 CRAC-02 → RACK-07 冷却故障链`
        : 'CRAC-02 制冷恢复，故障链解除',
    )
  }
    ...
    calls.forEach((call, index) => {
      ...
      switch (call.name) {
        case 'focus_hottest': {
          ...
          response.push(`定位 ${hottest.id} ${hottest.temp.toFixed(1)}°C`)
          break
        }
        case 'focus_rack':
          selectRack(call.arguments.rack_id)
          response.push(`聚焦 ${call.arguments.rack_id}`)
          break
        case 'inspect_node': {
        ...
        case 'mitigate_incident': {
          ...
          setMitigating(true)
          selectRack(call.arguments.rack_id)
          response.push(`执行 ${call.arguments.rack_id} 降载处置`)
          pushEvent('warning', `Agent 执行临时降载：${call.arguments.rack_id} → ${call.arguments.target_gpu || 68}%`)
          break
        }
        case 'reset_view': resetView(); break
        default:
          ok = false
          response.push('存在未识别工具')
      }
  ...
  const runAgent = async (text = agentInput) => {
    ...
    const latestRacks = racksRef.current
    const context = {
      racks: latestRacks.map(({ id, temp, gpu, power, status }) => ({ id, temp, gpu, power, status })),
      incident,
      selectedRack,
      selectedDevice,
    }
  ...
  const finishDemo = () => {
    ...
    setDemoMessage('演示完成：故障定位、降载、恢复制冷和前后对比已完成。')
    pushEvent('info', '导演模式演示完成，已生成处置前后总结')
    if (directorMode) setDirectorSummary(true)
  }
  ...
  const executeDemoStep = (index) => {
    ...
    if (index === AUTO_DEMO_STEPS.length - 1) {
      demoTimer.current = window.setTimeout(finishDemo, step.duration)
    } else {
  ...
  const prepareDemo = () => {
    ...
    setAirflow(true)
    resetView()
    setAgentMessage('导演模式已接管场景。镜头、字幕、HUD 和场景工具将按剧本自动运行。')
  }
  ...
  const startDirectorDemo = () => {
    setDirectorMode(true)
    prepareDemo()
    setDirectorIntro(true)
  ...
  const directorActive = directorMode && (demoRunning || directorIntro)
  const showFaultHud = directorActive && !directorIntro && demoStep >= 1

  return (
    <main className={`app-shell app-v4 app-v5 app-v6 ${directorActive ? 'director-active' : ''}`}>
      ...
      <header className="topbar glass-panel standard-ui">
        <div className="brand-block"><div className="brand-mark"><span /></div><div><p>DIGITAL TWIN / LIVE</p><h1>AI 数据中心数字孪生</h1></div></div>
        <div className="v4-mode-badge v5-mode-badge v6-mode-badge"><span />v0.6 · 导演模式</div>
      ...
      <section className="metrics-row standard-ui">
      ...
      <aside className="left-panel glass-panel v4-left-panel v5-left-panel v6-left-panel standard-ui">
        <div className="panel-kicker">演示控制</div><h2>智能故障处置链</h2>
        <button className="director-start-button" onClick={startDirectorDemo} disabled={demoRunning}>◉ 导演模式演示</button>
        <button className={`demo-start-button ${demoRunning ? 'running' : ''}`} onClick={demoRunning ? stopDemo : startDemo}>{demoRunning ? '停止当前演示' : '▶ 标准自动演示'}</button>
        ...
        <div className="controls-stack compact-controls">
          <Toggle active={heatmap} onClick={() => setHeatmap((v) => !v)}>热力图</Toggle>
          <Toggle active={links} onClick={() => setLinks((v) => !v)}>网络流</Toggle>
          <Toggle active={airflow} onClick={() => setAirflow((v) => !v)}>冷气流</Toggle>
        </div>
      </aside>

      <aside className="detail-panel glass-panel v4-detail-panel v5-detail-panel standard-ui">
        ...
        {selected && <>
          <div className="asset-hero">
            ...
            <div><span>{selectedDevice ? 'NODE GPU LOAD' : 'RACK GPU LOAD'}</span><strong>{selectedDevice ? '8 × H100 SXM' : '8 计算节点 · 64 × H100'}</strong><small>{selectedDevice || selected.id}</small></div>
          </div>
          ...
          <div className={`ai-card ${selected.status === 'critical' ? 'ai-card--critical' : ''}`}>
            ...
            <div><strong>{mitigating ? '正在执行降载处置' : selected.status === 'critical' ? '冷却故障影响扩大' : '状态可控'}</strong><p>{mitigating ? `目标 GPU 负载 ${targetGpu}%，风扇增速，等待温度回落。` : incident ? 'CRAC-02 风量异常，建议定位热点节点后执行临时降载。' : '当前可继续监视温度与计算负载。'}</p></div>
          </div>
          <SnapshotCompare before={beforeSnapshot} after={afterSnapshot} />
        </>}
      </aside>

      <section className="agent-console glass-panel v5-agent-console standard-ui">
        <div className="agent-console-head">
          <div><span className="agent-orb" /><div><strong>场景 Agent</strong><small>工具规划 · 根因分析 · 处置</small></div></div>
          <span className={`agent-runtime-state source-${plannerSource.toLowerCase()}`}>{plannerSource}</span>
        </div>
        <p className="agent-response">{agentMessage}</p>
        <div className="agent-quick-row">
          {['定位最热设备', '检查 RACK-07 GPU-6', '处置 RACK-07 降载到 68%', '恢复制冷'].map((text) => <button key={text} onClick={() => runAgent(text)} disabled={demoRunning}>{text}</button>)}
        </div>
        <form className="agent-input-row" onSubmit={(event) => { event.preventDefault(); runAgent() }}>
          <input value={agentInput} onChange={(event) => setAgentInput(event.target.value)} disabled={demoRunning} placeholder="例如：处置 RACK-07 降载到 65%" />
          <button type="submit" disabled={demoRunning}>执行</button>
        </form>
        <ToolCallLog items={toolCalls} />
      </section>

      <section className="event-panel glass-panel v4-event-panel v5-event-panel standard-ui">
        ...
        <div className="events-list">
          {events.slice(0, 3).map((event, index) => <div className="event-row" key={`${event.time}-${index}`}><span className={`event-level event-${event.level}`} /><time>{event.time}</time><p>{event.text}</p></div>)}
        </div>
      </section>

      <div className="corner-label standard-ui">DC-01 · ROOM A · DIGITAL TWIN 0.6</div>

      {directorActive && <div className="director-rec"><span />REC <b>DC-01 / AUTO DIRECTOR</b></div>}
      <DirectorIntro visible={directorIntro} />
      <DirectorCaption visible={directorActive && !directorIntro} stepIndex={demoStep} />
      <FaultHud visible={showFaultHud} rack={rack07} incident={incident} mitigating={mitigating} stepIndex={demoStep} />
      <DirectorProgress visible={directorActive && !directorIntro} stepIndex={demoStep} />
      <DirectorSummary visible={directorSummary} before={beforeSnapshot} after={afterSnapshot} onReplay={startDirectorDemo} onExit={exitDirectorMode} />
    </main>
  )
}
